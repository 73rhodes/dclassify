...
function calculateProbabilities(category, classifier) {
  ...
  */
  ...
  for (k in tokens) {
    ...
    probabilities[category] = probabilities[category] || {};
...
class Classifier {
  ...
  classify (item) {
    ...
    const firstPlace = categoryScores[0];
    const secondPlace = categoryScores[1];
    const timesMoreLikely = firstPlace.probability / secondPlace.probability;
    const probability = firstPlace.probability / sumOfProbabilities;
...
class Document {
  ...
  add (token) {
    ...
    } else if (typeof token === 'string' && this.tokens.indexOf(token) === -1) {
